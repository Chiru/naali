...
function ClientInitialize2() {
    // former ServerInitialize
    var rigidbody = me.GetOrCreateComponentRaw("EC_RigidBody");
    ...
    frame.Updated.connect(RotationUpdate);
...
function RotationUpdate(frametime) {
    if (!animsDetected) {
...
function ClientUpdatePhysics(frametime) {
    ...
    if (!flying) {
        ...
        if ((motion_x != 0) || (motion_y != 0)) {
            ...
            // print("apply impulse to av, active/auth: " + rigidbody.IsActive() + "/" + rigidbody.HasAuthority());
...
function ClientHandleMove(param) {
    ...
    // print("ClientHandleMove: " + param);

    // commented out because this crashes on client
    // var sender = server.GetActionSender();
    // if (sender)
    //     print("Move action from " + sender.GetName());
    // else
    //     print("Move action from unknown");
    if (param == "forward") {
        motion_x = 1;
    }
    if (param == "back") {
...
function ClientInitialize() {
    ...
    if (me.GetName() == "Avatar" + client.GetConnectionID()) {
        ...
        // for own avatar, do server's part
        ClientInitialize2()
    }
    else
    {
